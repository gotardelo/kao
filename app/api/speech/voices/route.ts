...
export async function POST(request: Request) {
  try {
    ...
    const data = await upstream.json() as { voices?: Array<{ voice_id?: string; name?: string }> };
    return Response.json({
      voices: (data.voices || []).slice(0, 100).map((voice) => ({ id: String(voice.voice_id || ''), name: String(voice.name || '') })),
